...
bilby_model.scale_likelihood = bilby_model.logLikelihoodEvaluation(inject)

Likelihood = bilby_model.getMinusLogLikelihood(np.vstack((np.ndarray.flatten(M1), np.ndarray.flatten(M2)))).reshape(ngrid, ngrid)
...
ax1.plot_surface(M1, M2, Likelihood, rstride = 3, cstride = 3, cmap=cm.coolwarm)
ax1.set_xlabel('mass_1')
ax1.set_ylabel('mass_2')
ax1.set_title('Normalized likelihood')
fig1.savefig('normalized_likelihood_surface.png')
